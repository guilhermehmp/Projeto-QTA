void manual() {
  ...
  if (digitalRead(redeon) != LOW) {  // REDE DESLIGADA
    ...
    if (digitalRead(ger_on) != LOW) {  // GERADOR DESLIGADO
      ...
      if (digitalRead(liga_ger) == HIGH || my_payload == "A") { // BOTÃO DE LIGA GERADOR
        ...
        delay(1000);
        my_payload = "b";
        if (digitalRead(ger_on) != HIGH) { // GERADOR LIGADO
    ...
    if (digitalRead(desliga_ger) == HIGH || my_payload == "a" ) { // BOTÃO DE DESLIGA GERADOR
