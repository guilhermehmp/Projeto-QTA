void automatico() {
  ...
  while (digitalRead(redeon) != LOW & i < 3) {
    ...
    client.publish("QTA_IoT_Envia_status_rede", String("Verificando falha na rede ou falta de energia").c_str()); //verificar qual valor retorna se der errado retire a função c_str
    delay(1000);
  }
  if (digitalRead(redeon) != LOW) {  // REDE DESLIGADA
    ...
    while (digitalRead(ger_on) != LOW & digitalRead(releger) == LOW & j < 3) {
      ...
      client.publish("QTA_IoT_Envia_status_gerador", String("Ligando Gerador partida automatica").c_str()); //verificar qual valor retorna se der errado retire a função c_str
    ...
    if (digitalRead(ger_on) != HIGH) {
      ...
      client.publish("QTA_IoT_Envia_status_gerador", String("Gerador Ligado").c_str()); //verificar qual valor retorna se der errado retire a função c_str
    } else {
      if(j<=6){
      delay(1000);
      j++;
      client.publish("QTA_IoT_Envia_status_gerador", String("!!! Atenção Gerador Não ligou Verificar ou não esta alimentando sistema !!! ").c_str()); //verificar qual valor retorna se der errado retire a função c_str
      ...
      client.publish("QTA_IoT_Envia_status_gerador", String("!!! Gerador não ligou na partida automatica  !!! ").c_str()); //verificar qual valor retorna se der errado retire a função c_str
  ...
  if (digitalRead(redeon) != HIGH) {  // REDE LIGADA
    ...
    client.publish("QTA_IoT_Envia_status_gerador", String("Gerador Desligado").c_str()); //verificar qual valor retorna se der errado retire a função c_str
    if (j == 0) {
      endtime = millis();
