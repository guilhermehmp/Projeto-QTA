void loop() {
  ...
  client.publish("QTA_IoT_Envia_emergencia", String(" ").c_str()); //verificar qual valor retorna se der errado retire a função c_str
   Serial.println("----------------------------------------------");
  ...
  if (digitalRead(emergencia) == LOW){
    ...
    client.publish("QTA_IoT_Envia_emergencia", String("!!! Botão de emergencia pressionado !!! ").c_str()); //verificar qual valor retorna se der errado retire a função c_str
